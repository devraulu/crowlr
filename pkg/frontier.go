...
func NewFrontier(maxSeen int) *Frontier {
	return &Frontier{
...
func (f *Frontier) Push(normalizedURL, originalURL, referrer string) {
	...
	if f.maxSeen > 0 && len(f.seen) >= f.maxSeen {
		slog.Warn("max seen reached, skipping", slog.Int("max_seen", f.maxSeen), slog.String("url", normalizedURL))
		return
	}
...
func (f *Frontier) Pop(defaultDelay time.Duration) (*Candidate, time.Duration) {
	...
	for host, hq := range f.queues {
		...
		if now.After(hq.NextVisit) {
			...
			slog.Info("frontier popped", slog.String("host", host), slog.String("url", url))
			seen := f.seen[url]
